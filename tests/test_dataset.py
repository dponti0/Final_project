...
class TestOutlierRemoval(unittest.TestCase):
    ...
    def test_remove_outliers(self, mock_isolation_forest, mock_read_csv):
        ...
        mock_isolation_forest.return_value.fit_predict.return_value = [1] * 8 + [
            -1,
            -1,
        ]  # Mocking two outliers in the last two rows
        ...
        result_df = remove_outliers(
            pd.read_csv("dummy_path.csv"), columns_to_remove_outliers
        )
