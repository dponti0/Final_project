...
class TestFilteringClass(unittest.TestCase):
    ...
    def test_filter_by_hypertension(self):
        ...
        self.assertEqual(filtered_df['hypertension'].sum(), len(filtered_df), "All individuals should have hypertension.")

    def test_filter_by_heart_disease(self):
        filtered_df = self.filtering_instance.filter_by_heart_disease()
        self.assertEqual(filtered_df['heart_disease'].sum(), len(filtered_df), "All individuals should have heart disease.")
    ...
    def test_filter_by_high_glucose(self):
        ...
        self.assertTrue(all(filtered_df['avg_glucose_level'] > glucose_threshold), "All individuals should have high glucose levels.")

    def test_filter_by_high_bmi(self):
        bmi_threshold = 30
        filtered_df = self.filtering_instance.filter_by_high_bmi(bmi_threshold)
        self.assertTrue(all(filtered_df['bmi'] > bmi_threshold), "All individuals should have high BMI.")

    def test_filter_by_stroke(self):
        filtered_df = self.filtering_instance.filter_by_stroke()
        self.assertEqual(filtered_df['stroke'].sum(), len(filtered_df), "All individuals should have had a stroke.")
