...
def determine_overall_health(row):
    ...
    if row["stroke"] == 1:
        ...
        and row["work_type"] == "private"
        and row["smoking_status"] == "smokes"
    ):
        return "Regular"
        ...
        and row["age"] >= 40
        and row["age"] <= 60
        and row["hypertension"] == 0
    ):
        return "Good"
    elif (
        row["work_type"] == "self-employed" and row["smoking_status"] == "never smoked"
    ):
        return "Good"
    elif 40 <= row["age"] <= 60:
        if (
            row["gender"] == "female"
            and row["heart_disease"] == 0
            and row["smoking_status"] == "never smoked"
        ):
            return "Very Good"
        elif (
            row["gender"] == "male"
            ...
            row["gender"] == "female"
            and row["heart_disease"] == 0
            and row["smoking_status"] == "formerly smoked"
        ):
            return "Good"
        elif (
            row["gender"] == "male"
