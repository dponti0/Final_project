...
class UnivariateVisualizer:
    ...
    def visualize_age_group_distribution(self):
        ...
        plt.figure(figsize=(10, 6))
        ax = sns.histplot(
            x="age_group", data=self.df, bins=20, kde=True, color="skyblue"
        )
    ...
    def visualize_gender_distribution(self):
        ...
        # Create the bar plot
        ax = sns.barplot(
            x=gender_percentages.values, y=gender_percentages.index, palette="pastel"
        )

        # Add % on the vertical axis
        for i, percentage in enumerate(gender_percentages):
            ax.text(
                percentage + 1, i, f"{percentage:.0f}%", va="center", fontweight="bold"
            )
    ...
    def visualize_work_type_distribution(self):
        ...
        plt.figure(figsize=(10, 6))
        ax = sns.countplot(
            x="work_type", data=self.df, palette=palette, edgecolor="none"
        )
    ...
    def visualize_smoking_distribution(self):
        ...
        ax = sns.countplot(
            x="smoking_status", data=self.df, palette=palette, edgecolor="none"
        )
